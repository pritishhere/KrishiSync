...
export const AgriPoolPage = () => {
  ...
  const renderRideCard = (ride) => (
    ...
    >
      {ride.verified && (
        <div className="absolute top-0 right-0 bg-linear-to-r from-emerald-500 to-green-600 text-white text-[10px] font-extrabold font-heading px-3 py-1 rounded-bl-xl uppercase tracking-wider shadow-md flex items-center gap-1">
      ...
      <div className="grid grid-cols-2 gap-3 bg-gray-50/70 p-3 rounded-xl border border-gray-200/80 text-xs">
        <div className="space-y-0.5">
          <span className="text-gray-500 font-medium flex items-center gap-1"><Scale size={12} /> Capacity</span>
          <strong className="text-[#1F2937] text-sm font-heading font-extrabold block">{ride.availableCapacity} kg</strong>
        </div>
        <div className="space-y-0.5">
          <span className="text-gray-500 font-medium flex items-center gap-1"><Truck size={12} /> Distance</span>
          <strong className="text-[#1F2937] text-sm font-heading font-extrabold block">{ride.distanceKm}</strong>
        </div>
        <div className="space-y-0.5 col-span-2">
          <span className="text-gray-500 font-medium flex items-center gap-1"><Map size={12} /> Destination</span>
          <strong className="text-[#1F2937] text-sm font-heading font-extrabold block">{ride.destination}</strong>
        </div>
      </div>
  ...
  const renderFindView = () => (
    <>
      {isLoading ? (
        ...
        <div className="bg-red-50 border border-red-200 rounded-2xl p-4 text-sm font-semibold text-red-600">⚠️ {error}</div>
      ) : rides.length > 0 ? (
        <div className="space-y-4">{rides.map(renderRideCard)}</div>
      ) : (
        <div className="ks-card p-8 text-center border border-gray-200">
          <div className="w-16 h-16 bg-emerald-50 rounded-full flex items-center justify-center text-emerald-600 mx-auto mb-3 border border-emerald-200"><Truck size={32} /></div>
          <h3 className="text-lg font-bold text-gray-800 font-heading">No Rides Found Nearby</h3>
    ...
    <div className="ks-card p-5 space-y-4">
        ...
        <form className="space-y-4" onSubmit={(e) => e.preventDefault()}>
            <div>
                ...
                <input type="number" id="capacity" placeholder="e.g., 500" className="w-full bg-gray-50 border border-gray-300 text-gray-900 font-bold rounded-xl p-3 outline-none focus:border-emerald-500 focus-visible:ring-2 focus-visible:ring-emerald-500" />
            </div>
            <div>
                <label htmlFor="destination" className="block text-sm font-bold text-gray-700 mb-1 font-heading">Destination Mandi</label>
                <input type="text" id="destination" placeholder="e.g., Sealdah Koley Market" className="w-full bg-gray-50 border border-gray-300 text-gray-900 font-bold rounded-xl p-3 outline-none focus:border-emerald-500 focus-visible:ring-2 focus-visible:ring-emerald-500" />
            </div>
  ...
  return (
    <div className="flex flex-col h-full bg-linear-to-br from-blue-50 via-emerald-50 to-lime-50 font-body">
      <PageHeader title="Agri-Pool Transport" showBack={false} />

      <div className="flex-1 p-4 space-y-4 overflow-y-auto pb-8">
        <div className="relative rounded-2xl overflow-hidden shadow-lg border border-emerald-200 h-48 bg-emerald-900 flex items-center justify-center text-emerald-300/50 flex-col p-4 text-center">
            <Map size={48} className="opacity-20"/>
            <p className="font-bold mt-2">Live Map of Nearby Rides</p>
            <span className="text-xs">(Map Integration Coming Soon)</span>
        </div>

        <div className="grid grid-cols-2 gap-2 bg-emerald-100/70 p-1 rounded-xl border border-emerald-200">
            <button onClick={() => setView('find')} className={`py-2 px-4 rounded-lg text-sm font-bold transition-all ${view === 'find' ? 'bg-white text-emerald-700 shadow' : 'text-gray-600'}`}>Find a Ride</button>
            <button onClick={() => setView('publish')} className={`py-2 px-4 rounded-lg text-sm font-bold transition-all ${view === 'publish' ? 'bg-white text-emerald-700 shadow' : 'text-gray-600'}`}>Publish a Ride</button>
        </div>
